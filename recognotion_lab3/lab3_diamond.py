...
def get_target_block(top_left, down_right, target_frame, windowSize):
    #    block = np.zeros((windowSize,windowSize))
    #    for i,j in it.product(range(hindex,hindex+windowSize), range(windex,windex+windowSize)):
    #        block[(i-hindex),(j-windex)] = image[i,j]
    #    return block
    #    block = image[hindex:hindex+windowSize,windex:windex+windowSize]
    #    return block
    num_rows = windowSize
    num_cols = windowSize

    # return array pixel values initialization
    #top_l_x, top_l_y, down_r_x, down_r_y = 0

    # target frame outside the image return 0 array
    #if (down_right[0] < 0 or down_right[1] < 0 or target_frame.shape[0] -1 < top_left[0]  or target_frame.shape[1] - 1 < top_left[1]):
    #    return np.zeros((1,1),dtype=np.uint8)

    #if(top_left[0]+windowSize-1>target_frame.shape[0]-1 or top_left[1]+windowSize-1>target_frame.shape[1]-1):
    #    return np.zeros((1,1),dtype=np.uint8)
    ...
    for i,j in it.product(range(np.int(top_left[0]),np.int(top_left[0])+windowSize),range(np.int(top_left[1]),np.int(top_left[1])+windowSize)):
        try:
            ...
            #block[(i-top_left[0]),(j-top_left[1])] = 0
    ...
    def predict_image(self, anchor_frame, target_frame):
        ...
        for (block_row, block_column) in it.product(range(0, self.height - (windowSize -1), windowSize), range(0, self.width - (windowSize- 1),
            ...
            for search_block_row, search_block_col in it.product(range(-searchWindowSize, searchWindowSize + windowSize),
                ...
                target_block = get_target_block(up_left_corner, down_right_corner, target_frame, windowSize)
...
class Diamond_searcher():
    ...
    def LDSP_search(self,global_start, anchor_block, makro_block):
        ...
        target_block = get_target_block(
            ...
            global_start[0][1] + int(searchWindowSize / 2) - int(windowSize / 2)),[0,0],
        ...
        for i in range(0, 8):
            target_block = get_target_block((global_start[0][0] + (LDSP_searching_points[i])[0] + int(searchWindowSize/2) - int(windowSize/2),
                                            global_start[0][1] + (LDSP_searching_points[i])[1] + int(searchWindowSize/2) - int(windowSize/2)),
                                            [0,0],makro_block, windowSize)
            if not (target_block.shape == (windowSize, windowSize)):
                continue
    ...
    def SDSP_search(self,start_point,anchor_block,makro_block):
        ...
        target_block = get_target_block(
            (start_point[0][0] + int(searchWindowSize / 2) - int(windowSize / 2),
             start_point[0][1] + int(searchWindowSize / 2) - int(windowSize / 2)), [0, 0],
            makro_block, windowSize)
        ...
        for i in range(0,4):
            target_block = get_target_block(
                ...
                start_point[0][1] + (SDSP_searching_points[i])[1] + int(searchWindowSize / 2) - int(windowSize / 2)),[0,0],
                makro_block, windowSize)
...
create_folder(path, '/Exhaustive_search/predicted_image')
create_folder(path, '/Diamond_search/predicted_image')
create_folder(path, '/Exhaustive_search/error_image')
...
def callDiamondSearch(v1,filename):
    ...
    while (anchor_no < total_frame_number):
        ...
        ds_predicted_image = ds.predict_image(anchor_frame=anchor_frame, target_frame=target_frame)
...
out = cv2.VideoWriter('project.avi', cv2.VideoWriter_fourcc(*'DIVX'), 24, (width,height))
...
#Exhaustive search
# ex = Exhaustive_searcher(windowSize=windowSize, searchWindowSize=arrangeSearchWindowSize, width=width,
#                          height=height)
# start = time.time()
# ES_total_MSE = 0
# ES_total_PSNR = 0
...
#     anchor_frame = v1[anchor_no]
#     print(anchor_no)
#     es_predicted_image = ex.predict_image(anchor_frame=anchor_frame, target_frame=target_frame)
#     MSE, PSNR = calculate_PSNR(filename, anchor_no, es_predicted_image, anchor_frame, 'ES')
#     ES_total_MSE = ES_total_MSE + MSE
#     ES_total_PSNR = ES_total_PSNR + PSNR
#     imsave(path + '/Exhaustive_search/predicted_image/' + get_name(filename, anchor_no, 'predicted'),
#            np.array(es_predicted_image, dtype=np.uint8))
#     anchor_no = anchor_no + 1
#     target_no = target_no + 1
# end = time.time()
...
# total_time = (end-start)/60
# mean_time = total_time/(total_frame_number-1)
# EX_mean_PSNR = ES_total_PSNR / (total_frame_number-1)
# EX_mean_MSE = ES_total_MSE / (total_frame_number-1)
# print('total number of frames processed: ', total_frame_number)
# print('window size: ', windowSize)
# print('search window size: ', searchWindowSize)
# print('total time for EX in minutes: ', '{0:.3f}'.format(round(total_time,2)))
# print('mean time for EX in minutes: ' '{0:.3f}'.format(round(mean_time,2)))
# print('mean PNSR for EX : ', '{0:.2f}'.format(round(EX_mean_PSNR,2)))
# print('mean MSE for EX : ', '{0:.2f}'.format(round(EX_mean_MSE,2)))