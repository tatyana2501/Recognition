...
for i in range(0,42):
    ...
    video.append(cv2.imread(r'C:\Users\Anna.Kravchenko\PycharmProjects\recognotion_lab3\ebma_frames\frames_of_interest{0}-{1}\error_image.png'.format(2*i,2*i+1)))
    video.append(cv2.imread(r'C:\Users\Anna.Kravchenko\PycharmProjects\recognotion_lab3\ebma_frames\frames_of_interest{0}-{1}\error_image.png'.format(2 * i,2 * i + 1)))


out = cv2.VideoWriter('output/ebma_error.avi', cv2.VideoWriter_fourcc(*'DIVX'), 24, (width,height))
